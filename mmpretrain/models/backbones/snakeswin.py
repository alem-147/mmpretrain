...
class SnakeSwinBlock(BaseModule):
    """Swin Transformer block.
    ...
    def __init__(self,
        ...
        super(SnakeSwinBlock, self).__init__(init_cfg)
        self.with_cp = with_cp
...
class SnakeSwinBlockSequence(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(depth):
            ...
            block = SwinBlock(**_swin_block_cfg)
            self.blocks.append(block)
            _vss_block_cfg = {
                'hidden_dim': vss_hidden_dims,
                'drop_paths': drop_paths[i], # fix this later
            }
            block = VSSBlock(**_vss_block_cfg)
            self.blocks.append(block)
...
class SwinBlockSequenceV2(BaseModule):
    """Module with successive Swin Transformer blocks and downsample layer.
    ...
    def __init__(self,
        ...
        for i in range(depth):
            ...
            block = SnakeSwinBlock(**_block_cfg)
            self.blocks.append(block)
...
class SnakeSwin(BaseBackbone):
    ...
    arch_zoo = {
        **dict.fromkeys(['t', 'tiny'],
                        {'embed_dims': 96,
                         'depths':     [1, 1,  3,  1],# sequences of swin + VSS
                         'num_heads':  [3, 6, 12, 24],
                         'vss_hidden_dims': [96, 192, 384, 768]}),
        **dict.fromkeys(['t2', 'tiny2'],
                        {'embed_dims': 96,
                         'depths': [1, 1, 3, 1],  # s2equences of swin + ss2d
                         'num_heads': [3, 6, 12, 24],
                         'vss_hidden_dims': [96, 192, 384, 768]}),
    ...
    def __init__(self,
                 ...
                 patch_cfg=dict(),
                 init_cfg=None):
        super(SnakeSwin, self).__init__(init_cfg=init_cfg)
        ...
        if isinstance(arch, str):
            ...
            essential_keys = {'embed_dims', 'depths', 'num_heads', 'vss_hidden_dims'}
        ...
        self.vss_hidden_dims = self.arch_settings['vss_hidden_dims']
        ...
        for i, (depth,
                num_heads,
                vss_hidden_dims) in enumerate(zip(self.depths, self.num_heads, self.vss_hidden_dims)):
            ...
            _stage_cfg = {
                ...
                'depth': depth,
                'num_heads': num_heads,
                'vss_hidden_dims': vss_hidden_dims,
                'window_size': window_size,
            ...
            stage = SnakeSwinBlockSequence(**_stage_cfg)
    ...
    def init_weights(self):
        super(SnakeSwin, self).init_weights()
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, *args,
        ...
        if (version is None
                or version < 2) and self.__class__ is SnakeSwin:
            ...
            for k in state_dict_keys:
                if k.startswith('norm.') or k.startswith('backbone.norm.'):
                    ...
                    del state_dict[k]
        if (version is None
                or version < 3) and self.__class__ is SnakeSwin:
            state_dict_keys = list(state_dict.keys())
            for k in state_dict_keys:
                if 'attn_mask' in k:
                    del state_dict[k]
    ...
    def train(self, mode=True):
        super(SnakeSwin, self).train(mode)
        self._freeze_stages()
